...
mod_tab2_server <- function(id, ctx, session) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$download_meta_template <- shiny::downloadHandler(
      filename = function() {
        paste0(ctx$data$dataset_name %||% "Dataset",
               "_xylo_meta_",
               Sys.Date(),
               ".xlsx")
      },
    ...
    output$download_example_meta <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        tp <- system.file(
          "extdata",
          "Ltal2007_xylo_meta_2025-09-01.xlsx",
          package = "xyloR"
        )
      ...
      meta <- tryCatch(
        ...
        error = function(e) {
          ...
          shiny::showNotification(
            paste("Meta file error:", conditionMessage(e)),
            type = "error",
            duration = 8
          )
          NULL
    ...
    observe({
      ...
      obs_ok <- is.data.frame(obs) && nrow(obs) > 0
      meta_ok <- is.list(meta) && !is.null(meta$site)
      
      ctx$data$tab2_ready <- obs_ok && meta_ok
    })
    
    validation_tbl <- reactive({
      req(isTRUE(ctx$data$tab2_ready))
    ...
    observe({
      ...
      ctx$data$validation_global <- tbl
    })
    
    observe({
      tbl <- ctx$data$validation_global %||% data.frame()
      ctx$data$tab2_validation_done <- is.data.frame(tbl)
    })
    ...
    observe({
      ...
      data_ok <-
        ...
        is.list(ctx$data$meta) &&
        !is.null(ctx$data$meta$site)
      
      valid <-
        is.data.frame(ctx$data$validation_global %||% data.frame()) &&
        nrow(ctx$data$validation_global %||% data.frame()) == 0
      
      cls <- if (data_ok && valid) "bg-success" else "bg-danger"
      ...
      shinyjs::runjs(sprintf(
        ...
        ns("zip_card"),
        if (data_ok && valid) "block" else "none"
      ))
    })
    ...
    output$validation_message <- shiny::renderUI({
      ...
      valid <- ctx$data$is_valid
      
      if (valid) {
        htmltools::div(
          class = "alert alert-success p-3 rounded mt-2",
          shiny::tags$h4(shiny::icon("check-circle"), " Validation passed!", class = "mb-2"),
          shiny::tags$p("Click ", shiny::tags$b("'Continue →'"),
                        " to proceed or download the ZIP below.",
                        class = "mb-0")
        )
      } else {
        htmltools::div(
          class = "alert alert-danger p-3 rounded mt-2",
          shiny::tags$h4(shiny::icon("exclamation-triangle"),
                         " Validation issues found",
                         class = "mb-2"),
          shiny::tags$p(
            "Fix the issues listed above, then re-upload your metadata file.",
            class = "mb-0"
          )
        )
      }
    ...
    df_hierarchy_reactive <- reactive({
      ...
      sheet_data <- setNames(
        lapply(sheet_names, function(s)
          readxl::read_excel(meta_path, sheet = s)[-1:-6, ]
        ),
        sheet_names
      )
      
      sheet_data[["sample"]]$sample_date <-
        as.Date(as.numeric(sheet_data[["sample"]]$sample_date),
                origin = "1899-12-30")
      ...
      df_joined <- dplyr::left_join(
        ...
        dplyr::left_join(sheet_data[["site"]],
                         by = "site_label",
                         relationship = "many-to-many") |>
        dplyr::mutate(
