...
# Install xyloR from GitHub
devtools::install_github("FAIRWood-datashare/xyloR")
```

Alternatively, install the package locally if you have the source files:

```r
# Install xyloR from a local source
install.packages("xyloR")
library(xyloR)
```
...
This vignette provides a clear guide to using `xyloR`, ensuring users can efficiently manage and format their data.
